...
export const endServingToken = async (localIp, tokenId, total_serving_time_display, counterId) => {
  try {
    const token = localStorage.getItem("auth_token");

    const res = await fetch(
      `http://${localIp}:8000/api/end-serving/${tokenId}?total_serving_time_display=${total_serving_time_display}&counter_id=${counterId}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: token ? `Bearer ${token}` : "",
...
export const noShowToken = async (localIp, tokenId, counterId) => {
  try {
    const token = localStorage.getItem("auth_token");

    const res = await fetch(
      `http://${localIp}:8000/api/no-show-serving/${tokenId}?counter_id=${counterId}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: token ? `Bearer ${token}` : "",
...
export const manuallCall = async (localIp, ticket, counterId) => {
  try {
    const token = localStorage.getItem("auth_token");

    const res = await fetch(
      `http://${localIp}:8000/api/manual-call/${ticket}?counter_id=${counterId}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: token ? `Bearer ${token}` : "",
