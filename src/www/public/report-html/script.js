...
if (!from || !to) {
    // alert("Both 'from' and 'to' parameters are required");
} else {
    fetch(
        `http://192.168.1.205:8000/api/get-new-summary-report?start_date=${from}&end_date=${to}`
    )
        ...
        .then(
            ({
                ...
                renderDonutChart(serviceDistribution);
                renderServiceLegend(serviceDistribution);
                renderServiceProgressBars(services);
                renderServiceTables(statsByServices);

                const dateSpan = document.getElementById("dateRange");
                const dateSpan2 = document.getElementById("dateRange2");
                const generatedDateSpan =
                    document.getElementById("generatedDate");
                const preparedBySpan = document.getElementById("preparedBy");
                const lastWeekComparisonProgressSpan = document.getElementById("last-week-comparison-progress");
                const lastWeekAvgProgressSpan = document.getElementById("last-week-avg-progress");
                const totalVisitsSpan = document.getElementById("total-visits");

                if (totalVisitsSpan) {
                    totalVisitsSpan.textContent = total_visits;
                }

                if (lastWeekAvgProgressSpan) {
                    lastWeekAvgProgressSpan.textContent = avgResponseTimeChange;
                }

                if (lastWeekComparisonProgressSpan) {
                    lastWeekComparisonProgressSpan.textContent = lastWeekComparison;
                }
                ...
                if (dateSpan2) {
                    dateSpan2.textContent = dates;
                }
                if (generatedDateSpan) {
                    generatedDateSpan.textContent = generatedDate;
                }
                if (preparedBySpan) {
                    preparedBySpan.textContent = preparedBy;
                }
            }
        )
        .catch((error) => {
...
`;
        ...
        footer.innerHTML = `
            <span>© 2025 Queue Management System</span>
...
async function renderServiceProgressBars(services) {
    try {
        ...
        const colors = ["bg-blue-500", "bg-purple-500", "bg-slate-400"]; // Extend if more services
...
async function renderActivityChart(activityChartData) {
    try {
...
async function renderDonutChart(serviceDistribution) {
    try {
