...
import Create from '@/components/Counter/Create';
import Edit from '@/components/Counter/Edit';
import { useForm } from '@inertiajs/react';

// Contact type definition (renamed to Counter for clarity based on context)
export type Counter = {
    id: number;
    name: string;
    description: string;
    // Assuming 'service' is an object based on accessorKey 'service.name'
    service: {
        name: string;
    } | null;
};
...
export const columns: ColumnDef<Counter>[] = [
...

    {
        ...
        cell: ({ row }) => {
            ...
            // Destructure useForm for delete and edit operations
            const {
                delete: destroy,
                setData,
                ...
                <div>
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="right-0 cursor-pointer">
                            <DropdownMenuItem onClick={() => handleEdit(counter)}>
                                <Pencil className="mr-2 h-4 w-4" /> Edit
                            </DropdownMenuItem>

                            <DropdownMenuItem onClick={() => handleDelete(counter.id)} className="cursor-pointer text-red-600 hover:text-red-700">
                                <Trash className="mr-2 h-4 w-4" /> Delete
                            </DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                    {/* The Edit component is placed here to be adjacent to the action dropdown */}
                    <Edit title={'Counter'} endpoint={'counters'} open={open} setOpen={setOpen} item={counter} />
                </div>
            );
        },
    },
...
interface CounterTableProps {
    items: {
        data: Counter[];
        per_page: number;
        // Include other pagination properties from Inertia/Laravel's Paginator if needed
    };
}

export default function CounterTable({ items }: CounterTableProps) {
    const { get } = useForm();
        ...
        <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <div className="flex w-full items-center justify-between rounded-lg">
                <div className="flex items-center">
                    {/* Filter Input */}
                    <Input
                        placeholder="Search..." // Changed to 'names' as filter is on 'name' column
                ...
                {/* Create Button */}
                <Create title="Counter" endpoint="counters" />
            </div>
